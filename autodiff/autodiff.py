...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        node_to_output_grad[node] = sum(node_to_output_grads_list[node])

        gradient_list = node.op.gradient(node, node_to_output_grad[node])
        for i in range(len(node.inputs)):
            if(node.inputs[i] not in node_to_output_grads_list):
                node_to_output_grads_list[node.inputs[i]] = [gradient_list[i]]
            else:
                node_to_output_grads_list[node.inputs[i]].append(gradient_list[i])
